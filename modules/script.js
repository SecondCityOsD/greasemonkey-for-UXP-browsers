 ...
 * during installation.  Resetting the timestamps clears the "has local
 * edits" state (_modifiedTime > _installTime), so the Add-ons Manager
 * entry stops showing the yellow-stripes overlay and the Automatic
 * Updates radio goes back to whatever the user had it set to.
 */
 ...
 * the first time an edit is observed — sets the per-script Automatic
 * Updates preference to DISABLE so the user's local changes aren't
 * silently overwritten on the next update check.  The user can re-enable
 * auto-updates via the Add-ons Manager radio, which then prompts them
 * (see applyBackgroundUpdates setter in addons.js).
 *
...
Script.prototype.isModified = function () {
  ...
  if (this._modifiedTime != this.file.lastModifiedTime) {
    this._modifiedTime = this.file.lastModifiedTime;
    // An edit that post-dates the install: switch off auto-updates for
    // safety and persist the change so config.xml reflects the new value
    // and the AOM radio shows "Off".
 ...
 * can still manually update edited scripts after a confirmation prompt.
 * This matches Violentmonkey's getScriptUpdateUrl semantics.
 *
 ...
 * Local edits flip checkRemoteUpdates to AUTOUPDATE_DISABLE via
 * isModified(), so auto-updates naturally switch off the moment the user
 * edits a script.
 *
