...
| `GM_download` / `GM.download` | `modules/thirdParty/GM_download.js` loaded as sub-script over `GM_xmlhttpRequest` | Native binary download with `nsIDownloader` / `nsIWebBrowserPersist`. Add `onprogress` events + a real `saveAs` UI. |
...
| modules/thirdParty/GM_download.js | THIRD-PARTY LIVE (polyfill) | UXP-era loadSubScript | keep until native lands (Phase 7) |
...
| **6** | XBL bindings audit — leave large bindings (cludes editor) alone; convert any small bindings to anonymous content + JS. Cosmetic / future-proofing. |
